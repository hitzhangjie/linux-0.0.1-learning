...
/**
 * hit.zhangjie@gmail.com
 * 2015-04-10 04:44:22 PM
 * 
 * here, embeded assembly code is used in this macro body.
 * first, __NR_##name this function or macro, will put the system interrupt
 * number into %eax, and then call 'int 80h' to send interrupt signal, then
 * the proper interrupt procedure will be executed, after that, the result
 * will be put into %eax.
 * and we will retrieve the value in %eax, so "=a"(__res) will put the value
 * in %eax to the variable '__res', and if __res is valid, return it,
 * otherwise, set the errno and return -1.
 *
 * also, we see, all errno are positive numbers, actually, 1,2,3,...255.
 * when system call fails, only return -1.
 */
